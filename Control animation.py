...
#Depth
ax = axs[0]
pcm = ax.pcolormesh(x, y, Domain.init_depth, cmap=cmaps[0], vmin = dmin, vmax = dmax)
ax.set_title('Depth')
cbar_1 = fig.colorbar(pcm, ax=ax)
#x-momentum
ax = axs[1]
pcm = ax.pcolormesh(x,y, init_momentum_x, cmap=cmaps[1], vmin = xmin, vmax = xmax)
ax.set_title('x-Momentum')
cbar_2 = fig.colorbar(pcm, ax=ax)
#y-momentum
ax = axs[2]
pcm = ax.pcolormesh(x,y, init_momentum_y, cmap=cmaps[1], vmin = ymin, vmax = ymax)
...
while Domain.sim_time <= t_end:
    ...
    cbar_3.remove()
    ax = axs[0]
    pcm = ax.pcolormesh(x,y, Domain.depths, cmap=cmaps[0], vmin = dmin, vmax = dmax)
    #ax.set_title('Depth')
    cbar_1 = fig.colorbar(pcm, ax=ax)
    #x-momentum
    ax = axs[1]
    pcm = ax.pcolormesh(x,y, Domain.x_momentums, cmap=cmaps[1], vmin = xmin, vmax = xmax)
    #ax.set_title('x-Momentum')
    cbar_2 = fig.colorbar(pcm, ax=ax)
    #y-momentum
    ax = axs[2]
    pcm = ax.pcolormesh(x,y, Domain.y_momentums, cmap=cmaps[1], vmin = ymin, vmax = ymax)
    #ax.set_title('y-Momentum')
    cbar_3 = fig.colorbar(pcm, ax=ax) 
